...
function _result(logBook, state) {
    ...
    for (var name in logBook.data) {
        if (logBook.data) {
            var height = parseInt(name.slice(0, -3)); // remove " cm"
            cleanedData[height] = logBook.data;
        }
    }
    ...
    } else if (_.filter(cleanedData, function (height, data) {return data.length >= 3;}).length < 4) {
    ...
    } else if (_.max(_.keys(cleanedData)) <= 100) {
    ...
    } else if (_.min(_.keys(cleanedData)) >= 50) {
    ...
    for (var height in cleanedData) {
        ...
        if (_.any(cleanedData[height], function (datum) {return Math.abs(avgs[height] - datum) > 300;})) {
    ...
    var correlation = util.correlation(transposed[0], transposed[1]);
    if (
